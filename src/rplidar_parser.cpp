...
namespace rplidar {
    ...
    std::size_t Parser::feed(const std::uint8_t* data, std::size_t len,
        ...
        while (buf_.size() >= 5) {
            if (!looks_valid(buf_.data())) {
                buf_.erase(buf_.begin());            // misaligned: drop 1, retry
                continue;
            }

            Measurement m = decode_record(buf_.data());
            buf_.erase(buf_.begin(), buf_.begin() + 5);
            ...
            if (m.start) {
                ...
                current_.push_back(m);               // begin the new rotation
